...
func NewRouter(config Config, db *gorm.DB, cache *redis.Client, session *discordgo.Session, client *s3.Client) *echo.Echo {
	router := &router{
		engine:  echo.New(),
		db:      db,
		cache:   cache,
...
func (r *router) setup() {
	...
	middleware := middleware.NewMiddleware(r.db)
	{
		...
		v1.NewV1Router(v1group, r.db, r.cache, r.client, r.session)
	}
}
