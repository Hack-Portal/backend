...
func (q *Queries) ListAccountPastWorksByOpus(ctx context.Context, opus int32) ([]AccountPastWorks, error) {
	rows, err := q.db.QueryContext(ctx, listAccountPastWorksByOpus, opus)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
