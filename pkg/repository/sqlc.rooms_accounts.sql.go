...
// versions:
//   sqlc v1.22.0
// source: sqlc.rooms_accounts.sql
...
func (q *Queries) CreateRoomsAccounts(ctx context.Context, arg CreateRoomsAccountsParams) (RoomsAccount, error) {
	row := q.db.QueryRow(ctx, createRoomsAccounts, arg.AccountID, arg.RoomID, arg.IsOwner)
	var i RoomsAccount
	err := row.Scan(
		&i.RoomsAccountID,
		&i.AccountID,
...
func (q *Queries) DeleteRoomsAccountsByID(ctx context.Context, arg DeleteRoomsAccountsByIDParams) error {
	_, err := q.db.Exec(ctx, deleteRoomsAccountsByID, arg.RoomID, arg.AccountID)
	return err
}
...
func (q *Queries) GetRoomsAccountsByID(ctx context.Context, roomID string) ([]GetRoomsAccountsByIDRow, error) {
	rows, err := q.db.Query(ctx, getRoomsAccountsByID, roomID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) ListJoinRoomByID(ctx context.Context, arg ListJoinRoomByIDParams) ([]ListJoinRoomByIDRow, error) {
	rows, err := q.db.Query(ctx, listJoinRoomByID, arg.AccountID, arg.Expired)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
